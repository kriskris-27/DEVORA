        ...
        const load = async () => {
            ...
            try {
                ...
                if (res.status === 404) {
                    ...
                    const profileData: Mechanic = {
                        email: mech.email,
                        name: mech.name,
                        workingHours: mech.workingHours || (mech.working_hours_from && mech.working_hours_to ? `${mech.working_hours_from} - ${mech.working_hours_to}` : ''),
                        phone: mech.phone,
                        location: mech.location,
                        vehicleTypes: mech.vehicleTypes || mech.vehicle_types || [],
                        startingPrice: mech.startingPrice || mech.starting_price || 0
                    }
                    ...
                    <>
                        {!isEditing ? (
                            <div className="mt-6 grid gap-3 max-w-xl">
                                ...
                                <div className="text-gray-700">
                                    <span className="font-medium">Starting Price:</span> {profile.startingPrice ? `$${profile.startingPrice}` : 'Not set'}
                                </div>
                                <div className="text-gray-700 flex items-center gap-2">
                                    <span className="font-medium">Vehicle Types:</span>
                                    <div className="flex gap-1">
                                        {profile.vehicleTypes?.length ? profile.vehicleTypes.map(t => (
                            ...
                            <div className="mt-6 grid gap-3 max-w-xl">
                                    ...
                                    <div className="flex flex-wrap gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                        {['Car', 'Bike', 'Heavy'].map((type) => (
                                            <label key={type} className="flex items-center gap-2 cursor-pointer group">
                                                <input
                                                    type="checkbox"
                                                    className="w-4 h-4 rounded border-gray-300 text-electric-600 focus:ring-electric-500"
                                                    checked={profile.vehicleTypes?.includes(type) || false}
                                                    onChange={(e) => {
                                                        const current = profile.vehicleTypes || []
                                                        const updated = e.target.checked
                                                            ? [...current, type]
                                                            : current.filter(t => t !== type)
                                                        setProfile({ ...profile, vehicleTypes: updated })
                                                    }}
                                                />
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. 50"
                                        onChange={(e) => setProfile({ ...profile, startingPrice: Number(e.target.value) || 0 })}
                                    />
                                </div>
